...
def main():
    ...
    for alpha in [0.50, 1.0, 1.50]:
        hooked_model = HookedTransformer.from_pretrained_no_processing(model_id).to(device)
        hf_transformer = AutoModelForCausalLM.from_pretrained(model_id).to(device)
...
if __name__ == "__main__":
    ...
    save_dir = PROJECT_ROOT / "models" / "llama_1b" / "orthogonalized"

    selected_layers = [9, 8, 10]  # [9, 8, 7, 10, 13, 14, 15]
    harm_tier = 2
    refusal_dict_path = PROJECT_ROOT / "models" / "llama_1b" / "refusal_analysis" / f"sequential_rv_dict_tier_{harm_tier}.pt"
    ...
    refusal_subspace = refusal_subspace.to(device)
