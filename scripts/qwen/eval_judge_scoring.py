...
def main():
    method = input("Test method ('sequential' or 'batched'): ").strip()
    assert method in ["sequential", "batched"], "method must be 'sequential' or 'batched'"

    input_path = (
        PROJECT_ROOT
        / "experiments"
        / "qwen_seq_vs_batch"
        / f"{method}_rollouts_evaluated.json"
    )
    ...
    output_path = (
        ...
        / "experiments"
        / "qwen_seq_vs_batch"
        / f"{method}_aggregate_metrics.json"
    )
