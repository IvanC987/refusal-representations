...
def main():
    method = input("Test method ('sequential', 'batched'): ")
    assert method in ['sequential', 'batched']
    rollout_path = PROJECT_ROOT / "experiments" / "qwen_seq_vs_batch" / f"{method}_rollouts.json"
    ...
    output_path = PROJECT_ROOT / "experiments" / "qwen_seq_vs_batch" / f"{method}_rollouts_evaluated.json"
