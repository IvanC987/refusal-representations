...
def sweep_layers(model_id: str, device: str, json_path: str, batch_size: int):
    ...
    for idx in tqdm(range(0, len(data), batch_size)):
        ...
        harmful_tokens = tokenize_instructions_qwen_chat([e[0] for e in messages], tokenizer).to(device)
        safe_tokens = tokenize_instructions_qwen_chat([e[1] for e in messages], tokenizer).to(device)
